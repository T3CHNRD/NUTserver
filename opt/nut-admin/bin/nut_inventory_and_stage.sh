...
SECRET_PATTERNS='password|passwd|secret|token|apikey|api_key|private_key|BEGIN[[:space:]]+RSA|BEGIN[[:space:]]+OPENSSH|Authorization:|vmware|netapp|proxmox|esxi|vcenter|smtp_pass|community=|bindpw='
...
# Hostnames, IP addresses, target mappings and infrastructure topology are
# required non-secret disaster-recovery information. The infra_flag remains
# inventory metadata but does not by itself prevent Git staging.
...
while IFS= read -r f; do
  ...
  if [[ "$secret_flag" == "yes" ]] || ! is_safe_allowlisted "$f"; then
    echo "$f" >> "$SKIP_LIST"
    continue
  fi
...
- Non-secret infrastructure details are retained for complete disaster recovery
